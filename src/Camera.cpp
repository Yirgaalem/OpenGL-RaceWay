...
Camera::Camera() {
	setCameraSky();
	viewup.set(0, 0, 1);
	aspect = 1.0, vangle = 50.0, dnear = 1.0, dfar = 150.0;
	setViewNorm();
}
...
void Camera::setCameraMiddle() {
	eye.set(0, 0, 10);
	ref.set(-7, -7, 2);

}
void Camera::setCameraCar() {
	eye.set(-7.0, 5.0, 3.0);
	ref.set(-7, -5, 1);
}
void Camera::setCameraTop() {
	eye.set(10, 10, 60);
	ref.set(1, 1, 10);
}
void Camera::setCameraSky() {
	eye.set(35.0, 35.0, 60.0);
	ref.set(0, 0, 10);
}
