...
void Shape::rotate(GLfloat angle, GLint axis) {
	GLfloat x0 = 0, y0 = 0, z0 = 0, rx = 0, ry = 0, rz = 0, tx = 0, ty = 0, tz =
			0;

	tx = this->getMC().mat[0][3];
	ty = this->getMC().mat[1][3];
	tz = this->getMC().mat[2][3];

//	cout << "before x:" << tx << " y:" << ty << " z:" << tz << "\n";
	translate(-tx, -ty, -tz);

	if (axis == 1) { //model rotate x
		rx = this->getMC().mat[0][0];
		ry = this->getMC().mat[1][0];
		rz = this->getMC().mat[2][0];
		this->rotate(x0, y0, z0, rx, ry, rz, angle);
	} else if (axis == 2) { //model rotate y
		rx = this->getMC().mat[0][1];
		ry = this->getMC().mat[1][1];
		rz = this->getMC().mat[2][1];
		this->rotate(x0, y0, z0, rx, ry, rz, angle);
	} else if (axis == 3) {  //model rotate z
